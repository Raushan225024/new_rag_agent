...
from ingestion.store_content import upload_content
load_documents()
print(f"document: {len(config.document)}")
create_chunks()
print(f"chunks:{len(config.chunks)}")
print(f"chunks:{type(config.chunks)}")
print(f"chunks:{type(config.chunks[0])}")
print(f"chunks:{config.chunks[0]}")
...
upload_content(config.chunks, batch_size=1)
